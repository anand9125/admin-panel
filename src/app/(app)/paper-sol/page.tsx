...
import { PaperSolManager } from "@/components/paper/paper-sol-manager";
...
import { getPaperSeedDefault } from "@/lib/server/paper-api";

export const metadata = { title: "Paper SOL — Trenchers Admin" };
...
export default async function PaperSolPage({ searchParams }: { searchParams: Promise<{ env?: string }> }) {
  const env = resolveEnv((await searchParams).env);
  const configured = isPlatformConfigured(env);

  let users: AdminUser[] = [];
  let total = 0;
  let defaultLamports: number | null = null;
  let error: string | null = null;

  if (configured) {
    try {
      const [usersRes, def] = await Promise.all([
        listUsers(env, { limit: 200 }),
        getPaperSeedDefault(env).catch(() => ({ lamports: null })), // backend may not expose yet
      ]);
      users = usersRes.users;
      total = usersRes.total;
      defaultLamports = def.lamports;
    } catch (e) {
  ...
  return (
    <div className="rise space-y-6">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Paper SOL</h1>
        <p className="mt-0.5 max-w-xl text-sm text-muted">
          Set how much simulated SOL users get to practice with. Change the default for everyone,
          or give specific users a custom amount.
        </p>
      </div>

      {!configured ? (
        <>
          <TabsEnv tab="paper" env={env} />
        ...
        <PaperSolManager env={env} users={users} total={total} defaultLamports={defaultLamports} envSwitch={<TabsEnv tab="paper" env={env} />} />
      )}
    </div>
  );
}
